...
import io.qzz.nekobounce.features.module.modules.combat.Backtrack.runWithSimulatedPosition
import io.qzz.nekobounce.features.module.modules.player.Blink
import io.qzz.nekobounce.features.module.modules.world.Fucker
import io.qzz.nekobounce.features.module.modules.world.Nuker
import io.qzz.nekobounce.features.module.modules.world.scaffolds.*
import io.qzz.nekobounce.utils.attack.CPSCounter
import io.qzz.nekobounce.utils.attack.CooldownHelper.getAttackCooldownProgress
import io.qzz.nekobounce.utils.attack.CooldownHelper.resetLastAttackedTicks
...
import io.qzz.nekobounce.utils.inventory.ItemUtils.isConsumingItem
...
import io.qzz.nekobounce.utils.render.RenderUtils.drawCircle
import io.qzz.nekobounce.utils.render.RenderUtils.drawEntityBox
import io.qzz.nekobounce.utils.render.RenderUtils.drawPlatform
import io.qzz.nekobounce.utils.rotation.RandomizationSettings
import io.qzz.nekobounce.utils.rotation.RaycastUtils.raycastEntity
import io.qzz.nekobounce.utils.rotation.RaycastUtils.runWithModifiedRaycastResult
...
import io.qzz.nekobounce.utils.rotation.RotationUtils.getVectorForRotation
import io.qzz.nekobounce.utils.rotation.RotationUtils.isRotationFaced
import io.qzz.nekobounce.utils.rotation.RotationUtils.isVisible
import io.qzz.nekobounce.utils.rotation.RotationUtils.rotationDifference
import io.qzz.nekobounce.utils.rotation.RotationUtils.searchCenter
...
import io.qzz.nekobounce.utils.timing.TimeUtils.randomClickDelay
...
object KillAura : Module("KillAura", Category.COMBAT) {
    ...
    private val adaptiveBezierSpeed by float("AdaptiveBezierTurnSpeed", 0.2f, 0.01f..1f)
    private val adaptiveSlerpSpeed by float("AdaptiveSlerpTurnSpeed", 0.2f, 0.01f..1f)
    private val sinusoidalSpeed by float("SinusoidalTurnSpeed", 0.2f, 0.01f..1f)
    private val springSpeed by float("SpringTurnSpeed", 0.2f, 0.01f..1f)
    private val cosineInterpolationSpeed by float("CosineInterpolationTurnSpeed", 0.2f, 0.01f..1f)
    private val logarithmicInterpolationSpeed by float("LogarithmicInterpolationTurnSpeed", 0.2f, 0.01f..1f)
    private val elasticSpringSpeed by float("ElasticSpringTurnSpeed", 0.2f, 0.01f..1f)
    private val bezierSpeed by float("BezierTurnSpeed", 0.2f, 0.01f..1f)
    ...
    private val basicSimulationSpeedMultLarge by float("BasicSimulation-SpeedMult-Large", 1.8f, 1.0f..3.0f)
    private val basicSimulationSpeedMultMedium by float("BasicSimulation-SpeedMult-Medium", 1.2f, 0.8f..2.0f)
    private val basicSimulationSpeedMultSmall by float("BasicSimulation-SpeedMult-Small", 0.8f, 0.5f..1.5f)

    // AdvancedSimulation参数  
    private val advancedSimulationSpeed by float("AdvancedSimulationSpeed", 0.2f, 0.01f..1f)
    private val advancedSimulationBaseSpeed by float("AdvancedSimulation-BaseSpeed", 180f, 50f..360f)

    // 随机速度
    private val randomSpeedValue by boolean("RandomSpeed", true)
    private val randomSpeedFrequency by int("RandomSpeedFrequency", 1, 1..10)
            ...
            if (swing && failSwing) {
                ...
                runWithModifiedRaycastResult(rotation, range.toDouble(), throughWallsRange.toDouble()) {
                    ...
                    val isAnyClientGuiActive = mc.currentScreen?.javaClass?.`package`?.name?.contains(
                        NekoBounce.CLIENT_NAME, ignoreCase = true
                    ) == true
    ...
    private fun updateRotations(entity: Entity): Boolean {
        ...
        val finalRotation = when (rotationMode) {
            "NekoBounce" -> rotation
            else -> calculateAuraRotation(rotation, player, entity)
        }
     ...
     * Aura转头算法计算
     */
    private fun calculateAuraRotation(targetRotation: Rotation, player: EntityPlayer, target: Entity): Rotation {
        ...
        when (rotationMode) {
            "Smooth" -> {
                ...
                val yaw = calculateSmoothYaw(currentRot.yaw, targetRotation.yaw, speed, target as? EntityPlayer)
                val pitch = calculateSmoothPitch(currentRot.pitch, targetRotation.pitch, speed)
                return Rotation(yaw, pitch)
            }
            "BasicSimulation" -> {
                val yaw = calculateBasicSimulationYaw(currentRot.yaw, targetRotation.yaw, target as? EntityPlayer)
                val pitch = calculateBasicSimulationPitch(currentRot.pitch, targetRotation.pitch)
                return Rotation(yaw, pitch)
            }
            "AdvancedSimulation" -> {
                val yaw = calculateAdvancedSimulationYaw(currentRot.yaw, targetRotation.yaw, target as? EntityPlayer)
                val pitch = calculateAdvancedSimulationPitch(currentRot.pitch, targetRotation.pitch)
                return Rotation(yaw, pitch)
            }
            else -> return targetRotation
        }
    }
     ...
     * Smooth模式Yaw计算
     */
    private fun calculateSmoothYaw(current: Float, target: Float, speed: Float, targetPlayer: EntityPlayer?): Float {
    ...
    /**
     * BasicSimulation模式Yaw计算
     */
    private fun calculateBasicSimulationYaw(current: Float, target: Float, targetPlayer: EntityPlayer?): Float {
        val delta = MathHelper.wrapAngleTo180_float(target - current)
    ...
    /**
     * AdvancedSimulation模式Yaw计算
     */
    private fun calculateAdvancedSimulationYaw(current: Float, target: Float, targetPlayer: EntityPlayer?): Float {
        val delta = MathHelper.wrapAngleTo180_float(target - current)
        val currentTime = System.currentTimeMillis()
        ...
        var newYaw = current + advancedSimulationCurrentSpeed * dt
