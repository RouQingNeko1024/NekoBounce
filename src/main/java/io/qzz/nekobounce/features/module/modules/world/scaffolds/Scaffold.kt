 ...
 */
package io.qzz.nekobounce.features.module.modules.world.scaffolds
...
object Scaffold : Module("Scaffold", Category.WORLD, Keyboard.KEY_I) {
    ...
    // Hide Sneak (新增选项)
    private val hideSneak by boolean("HideSneak", false) { eagle == "Normal" && scaffoldMode != "GodBridge" }
    ...
    fun onCameraSetup(event: EntityViewRenderEvent.CameraSetup) {
        if (hideSneak && mc.thePlayer != null && mc.thePlayer.isSneaking) {
            // 将角度转换为弧度
            val pitch = Math.toRadians(event.pitch.toDouble())
            val yOffset = -0.08 * cos(pitch)
            val zOffset = -0.08 * sin(pitch)

            net.minecraft.client.renderer.GlStateManager.translate(0.0, yOffset, zOffset)
        }
    }
