...
class Arraylist(
    ...
    private val tagsArrayColor by color("TagsArrayColor", Color.GRAY, rainbow = true) {
        tags
    }.onChanged { updateTagDetails() }
    ...
    fun updateTagDetails() {
        ...
        tagPrefix = " §${getColorCode(tagsArrayColor)}" + pair.first
        tagSuffix = pair.second
    }

    private fun getColorCode(color: Color): String {
        // 将Color转换为最接近的Minecraft颜色代码
        val r = color.red
        val g = color.green
        val b = color.blue
        ...
        val minecraftColors = mapOf(
            "0" to Color(0, 0, 0),         // 黑色
            "1" to Color(0, 0, 170),       // 深蓝
            "2" to Color(0, 170, 0),       // 深绿
            "3" to Color(0, 170, 170),     // 深青
            "4" to Color(170, 0, 0),       // 深红
            "5" to Color(170, 0, 170),     // 紫色
            "6" to Color(255, 170, 0),     // 金色
            "7" to Color(170, 170, 170),   // 灰色
            "8" to Color(85, 85, 85),      // 深灰
            "9" to Color(85, 85, 255),     // 蓝色
            "a" to Color(85, 255, 85),     // 亮绿
            "b" to Color(85, 255, 255),    // 天蓝
            "c" to Color(255, 85, 85),     // 红色
            "d" to Color(255, 85, 255),    // 粉红
            "e" to Color(255, 255, 85),    // 黄色
            "f" to Color(255, 255, 255)    // 白色
        )
        
        var minDistance = Double.MAX_VALUE
        var closestCode = "7" // 默认灰色
        
        for ((code, mcColor) in minecraftColors) {
            val distance = Math.sqrt(
                Math.pow((r - mcColor.red).toDouble(), 2.0) +
                Math.pow((g - mcColor.green).toDouble(), 2.0) +
                Math.pow((b - mcColor.blue).toDouble(), 2.0)
            )
