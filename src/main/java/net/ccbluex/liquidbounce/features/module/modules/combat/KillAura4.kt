...
object KillAura4 : Module("KillAura4", Category.COMBAT) {
    /**
     * OPTIONS
     */
    ...
    private var blockRange: Float by float("BlockRange", range, 1f..8f).onChange { _, new ->
        new.coerceAtMost(this@KillAura4.range)
    }
    ...
    // 自动追踪选项 - 简化版本
    private val autoTrack by boolean("AutoTrack", true)
    private val autoTrackJump by boolean("AutoTrackJump", true)
    ...
    // Swing fails
    private val swingFails = mutableListOf<KillAura2SwingFailData>()
    ...
    val onTick = handler<GameTickEvent>(priority = 2) {
        ...
        if (target != null) {
            ...
            val generatedClicks = if (generateClicksBasedOnDist) {
                ...
                ((distance / distanceFactor.random()) * cpsMultiplier.random()).toInt()
            } else 0
    ...
    val onUpdate = handler<UpdateEvent> {
        // 自动追踪逻辑：有目标时向前移动并跳跃
        if (autoTrack) {
            if (target != null && !shouldPrioritize()) {
                // 有目标，按下向前键
                mc.gameSettings.keyBindForward.pressed = true

                // 自动跳跃
                if (autoTrackJump && mc.thePlayer != null) {
                        ...
                        if (it.typeOfHit.isEntity) {
                            ...
                            if (renderBoxOnSwingFail) {
                                synchronized(swingFails) {
                                    ...
                                    swingFails += KillAura2SwingFailData(spot, System.currentTimeMillis())
                                }
                            }
                        }
                    }
...
// SwingFailData类 - 改为 KillAura2SwingFailData 避免重复声明
data class KillAura2SwingFailData(val vec3: Vec3, val startTime: Long)