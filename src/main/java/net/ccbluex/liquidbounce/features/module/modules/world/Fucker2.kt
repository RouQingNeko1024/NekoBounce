...
object Fucker2 : Module("Fucker2", Category.WORLD) {
    ...
    // 新增跳跃冻结功能设置
    private val freezeAtJumpHeight by boolean("FreezeAtJumpHeight", false)
    private val autoClose by boolean("AutoClose", false)
    private val autoCloseDelay by float("AutoCloseDelay", 0.5F, 0F..5F) { autoClose }
    ...
    private var jumpMaxY = Double.MIN_VALUE
    ...
    val onRotationUpdate = handler<RotationUpdateEvent> {
        ...
        if (freezeAtJumpHeight) {
            if (!jumpStarted) {
                ...
                // 追踪跳跃最高点
                if (player.posY > jumpMaxY) {
                    jumpMaxY = player.posY
                } else if (!shouldFreeze && Math.abs(player.posY - jumpMaxY) < 0.01) {
                    // 达到最高点，准备冻结
                    shouldFreeze = true
                    frozenX = player.posX
                    frozenY = player.posY
                ...
                if (shouldFreeze) {
                    ...
                    player.motionX = 0.0
                    player.motionY = 0.0
                    player.setPosition(frozenX, frozenY, player.posZ)
                }
            }
        }
    ...
    val onUpdate = handler<UpdateEvent> {
        ...
        // 如果启用了跳跃冻结功能，保持冻结状态
        if (freezeAtJumpHeight && shouldFreeze) {
            player.motionX = 0.0
            player.motionY = 0.0
            player.setPosition(frozenX, frozenY, player.posZ)
        }
        ...
        // 处理自动关闭延迟
        if (shouldAutoClose) {
            autoCloseTimer += mc.timer.timerSpeed
