...
object JumpCircle2 : Module("JumpCircle2", Category.RENDER) {
    ...
    private val innerColor by color("InnerColor", Color(0, 0, 0, 50)) { colorMode == "Custom" }
    private val outerColor by color("OuterColor", Color(0, 111, 255, 255)) { colorMode == "Custom" }
    ...
    private val staticLoc = "liquidbounce/textures/jumpcircle/default"
    private val animatedLoc = "liquidbounce/textures/jumpcircle/animated"
        ...
        try {
            ...
            setupDrawCircles {
                circles.removeIf { circle ->
                    val progress = ((runTimeTicks + partialTick) - circle.endTime) / lifeTime.toFloat()
                    ...
                    if(useTexture){
                        renderTexturedCircle(
                            ...
                            radius.toDouble(),
                            1f - progress,
                            circles.indexOf(circle) * 30,
                            lightFactor,
                            effectStrength
                        )
                    } else {
                        renderSimpleCircle(circle.pos, radius, progress)
                    }
                    progress >= 1F
                }
            }
    ...
    private fun renderSimpleCircle(pos: Vec3, radius: Float, timeFraction: Float){
        val (color, color2) = when (colorMode) {
            "Theme" -> {
                ...
                val inner = animateColor(baseColor, 1f - timeFraction)
                val outer = animateColor(baseColor, 1f - timeFraction)
                Pair(inner, outer)
            }
            else -> {
    ...
    private fun animateColor(baseColor: Color, progress: Float): Color {
        val alpha = (baseColor.alpha * (1 - progress)).toInt().coerceIn(0, 255)
        val color = Color(baseColor.red, baseColor.green, baseColor.blue, alpha)
        if (hueOffsetAnim == 0) {
            return color
        }
