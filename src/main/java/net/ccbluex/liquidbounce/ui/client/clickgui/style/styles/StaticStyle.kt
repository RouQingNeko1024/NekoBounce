...
object StaticStyle : Style() {
    override fun drawPanel(mouseX: Int, mouseY: Int, panel: Panel) {
        ...
        // 绘制面板背景
        RenderUtils.drawRect(x, y, x + width, y + height, Color(20, 20, 20, 150).rgb)

        // 绘制面板标题
        fontSemibold35.drawString(
            panel.name, 
            x + 4, 
            y + height / 2 - fontSemibold35.FONT_HEIGHT / 2, 
            Color.WHITE.rgb
        )
        ...
        fontSemibold35.drawString(
            if (panel.open) "-" else "+", 
            x + width - 10, 
            y + height / 2 - fontSemibold35.FONT_HEIGHT / 4, 
            Color.WHITE.rgb
        )
        ...
        if (panel.open) {
            ...
            // 计算滚动偏移
            val scrollOffset = if (panel.scrollbar) panel.getVisibleRange().first * 12 else 0

            for (i in panel.getVisibleRange()) {
                val element = panel.elements[i]
                element.setLocation(x, yPos - scrollOffset)
                element.width = width

                if (yPos - scrollOffset >= y + height && yPos - scrollOffset < y + height + panel.fade) {
                    if (element is ModuleElement) {
                        drawModuleElementAndClick(mouseX, mouseY, element, null)
                    } else if (element is ButtonElement) {
                        drawButtonElement(mouseX, mouseY, element)
                    }
                }
            ...
            // 绘制滚动条
            if (panel.scrollbar) {
                val maxScroll = panel.elements.size - ClickGUI.maxElements
                val scrollPos = if (maxScroll > 0) (panel.getVisibleRange().first / maxScroll.toFloat() * (panel.fade - 20)).roundToInt() else 0

                RenderUtils.drawRect(
                    x + width - 6, 
                    y + height + 4 + scrollPos, 
                    x + width - 2, 
                    y + height + 24 + scrollPos, 
                    Color(100, 100, 100, 150).rgb
                )
            }
        }

        // 绘制面板边框
        RenderUtils.drawRect(x, y, x + width, y + 1, Color(100, 100, 100, 200).rgb)
        RenderUtils.drawRect(x, y + height - 1, x + width, y + height, Color(100, 100, 100, 200).rgb)
        RenderUtils.drawRect(x, y, x + 1, y + height, Color(100, 100, 100, 200).rgb)
        RenderUtils.drawRect(x + width - 1, y, x + width, y + height, Color(100, 100, 100, 200).rgb)
    }
    ...
    override fun drawHoverText(mouseX: Int, mouseY: Int, text: String) {
        ...
        val x = mouseX.clamp(0, (scaledWidth / ClickGUI.scale - width).roundToInt())
        val y = mouseY.clamp(0, (scaledHeight / ClickGUI.scale - height).roundToInt())
    ...
    override fun drawButtonElement(mouseX: Int, mouseY: Int, buttonElement: ButtonElement) {
        ...
        val width = buttonElement.width
        val height = buttonElement.height

        // 绘制按钮背景
        val backgroundColor = if (buttonElement.isHovered(mouseX, mouseY)) Color(50, 50, 50, 150) else Color(30, 30, 30, 150)
        RenderUtils.drawRect(x, y, x + width, y + height, backgroundColor.rgb)
        ...
        fontSemibold35.drawString(
            ...
            x + 2, 
            y + height / 2 - fontSemibold35.FONT_HEIGHT / 2, 
            if (buttonElement.color == Int.MAX_VALUE) Color.WHITE.rgb else buttonElement.color
        )
    }
    ...
    override fun drawModuleElementAndClick(
        ...
        if (moduleValues.isNotEmpty()) {
            ...
            if (moduleElement.showSettings) {
                ...
                if (moduleElement.settingsWidth > 0 && moduleElement.settingsHeight > 0) drawBorderedRect(
                    minX.toInt(), yPos.toInt(), maxX.toInt(), yPos + moduleElement.settingsHeight.toInt(), 1, 0, Int.MIN_VALUE
                )
                ...
                for (value in moduleValues) {
                    ...
                    when (value) {
                        ...
                        is FloatValue -> {
                            ...
                            val sliderValue =
                                (moduleElement.x + moduleElement.width + (moduleElement.settingsWidth - 12) * (displayValue - value.minimum) / (value.maximum - value.minimum)).toInt()
                        ...
                        is IntRangeValue -> {
                            ...
                            drawRect(8 + sliderValue1.toInt(), yPos + 15, sliderValue1.toInt() + 11, yPos + 21, guiColor)
                            drawRect(8 + sliderValue2.toInt(), yPos + 15, sliderValue2.toInt() + 11, yPos + 21, guiColor)
                        ...
                        is ColorValue -> {
                            ...
                            chosenText?.let {
                                ...
                                if (it.selectionActive()) {
                                    ...
                                    drawRect(
                                        ...
                                        (cursorY + fontSemibold35.fontHeight - 2).toInt(),
                                        Color(7, 152, 252).rgb
                                    )
                                }
                            ...
                            drawBorderedRect(
                                colorPreviewX1.toFloat(),
                                colorPreviewY1.toFloat(),
                                colorPreviewX2.toFloat(),
                                colorPreviewY2.toFloat(),
                                1.5f,
                                normalBorderColor,
                                value.get().rgb
                            )
                            ...
                            drawBorderedRect(
                                rainbowPreviewX1.toFloat(),
                                colorPreviewY1.toFloat(),
                                rainbowPreviewX2.toFloat(),
                                colorPreviewY2.toFloat(),
                                1.5f,
                                rainbowBorderColor,
                            ...
                            chosenText?.let {
                                ...
                                if (it.selectionActive()) {
                                    val start =
                                        textX - 1 + fontSemibold35.getStringWidth(it.string.take(it.selectionStart!!))
                                    val end = textX - 1 + fontSemibold35.getStringWidth(it.string.take(it.selectionEnd!!))
                                    drawRect(
                                        start.toInt(),
                                        (textY - 3).toInt(),
                                        end.toInt(),
                                        (textY + fontSemibold35.fontHeight - 2).toInt(),
                                        Color(7, 152, 252).rgb
                                    )
                                }
        ...
        // 处理模块点击事件
        if (mouseButton != null && mouseButton == 0 && mouseX in moduleElement.x..moduleElement.x + moduleElement.width && mouseY in moduleElement.y..moduleElement.y + moduleElement.height) {
