...
object Velocity : Module("Velocity", Category.COMBAT) {
    ...
    // New Option: AntiS12
    private val antiS12Value by boolean("AntiS12", false)

    private val horizontal by float("Horizontal", 0F, -2F..2F) { mode in arrayOf("Simple", "AAC", "Legit") }
    private val vertical by float("Vertical", 0F, -2F..2F) { mode in arrayOf("Simple", "Legit") }
    ...
    // SmartJumpReset 及其子选项（仅在SmartJumpReset模式下显示）
    ...
    val onUpdate = handler<UpdateEvent> {
        ...
        when (mode.lowercase()) {
            ...
            "matrixreduceplus" -> {
                // MatrixReducePlus模式处理移动到onPacket事件中
            }
            ...
            "predictiona" -> {
                // PredictionA模式：结合Jump和Click模式的特性
                if (hasReceivedVelocity) {
                    // Jump模式部分：固定参数
    ...
    val onGameTick = handler<GameTickEvent> {
        ...
        // PredictionA模式：Click模式部分
        if (mode == "PredictionA") {
            // 固定参数：hurtTimeToClick=10, ignoreBlocking=true, whenFacingEnemyOnly=false, clickRange=3, swingMode="Normal"
            ...
            if (target == null) {
                // whenFacingEnemyOnly = false，所以不需要检查视线
    ...
    val onPacket = handler<PacketEvent>(priority = 1) { event ->
        ...
        // AntiS12 Logic
        if (antiS12Value && packet is S12PacketEntityVelocity && packet.entityID == thePlayer.entityId) {
            event.cancelEvent()
            return@handler
        }
...
//        ) {
        ...
        if ((packet is S12PacketEntityVelocity && thePlayer.entityId == packet.entityID)
            ...
            when (mode.lowercase()) {
                ...
                "aac", "reverse", "smoothreverse", "aaczero", "ghostblock", "intavereduce", "predictiona",
                "intave14", "intave/polar-flag", "smartjumpreset" -> hasReceivedVelocity = true
