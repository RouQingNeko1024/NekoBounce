...
// Import both static and class to fix GL11 ref errors
...
object WaterMark2 : Module("WaterMark2", Category.RENDER) {
        ...
        if (ModuleNotify) {
            for (module in ModuleManager) {
                ...
                if (prevState != currentState) {
                    ...
                    val modName = "§a${module.name}"
                    val stateText = if (currentState) "§aEnabled" else "§cDisabled"
                    val message = "$modName §fhas been $stateText§f!"
    ...
    val onRender2D = handler<Render2DEvent> {
        ...
        val chestGui = mc.currentScreen is GuiChest && ChestTheme
        ...
        if (scaffoldModule?.state == true && isScaffold) {
            renderMode = "SCAFFOLD"
            targetWidth = 190F
            targetHeight = 58F
        ...
        if (renderMode != "NONE") {
            ...
            val currentRadius = if (AnimGlobalHeight > 30F) 8F else AnimGlobalHeight / 2F
    ...
    private fun renderScaffoldContent(x: Float, y: Float, w: Float, h: Float) {
        ...
        val bpsText = String.format("%.2f", if(AnimatedBps < 0.01) 0.00 else AnimatedBps)
    ...
    private fun drawChest() {
        ...
        val slotSize = 16
        val padding = 8
        val w = columns * slotSize + padding * 2
        val h = rows * slotSize + padding * 2
        val x = (width - w).toFloat() / 2
        val y = start_y.coerceIn(5f, height - h.toFloat() - 5f)
        drawRoundedRect(x, y, x+w, y+h, Color(0, 0, 0, BackgroundAlpha).rgb, ChestRounded)
        drawChestItems(chestSlots, x + padding, y + padding, slotSize)
    }
